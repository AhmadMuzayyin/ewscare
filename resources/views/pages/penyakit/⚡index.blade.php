...
new #[Title('Data Penyakit')] class extends Component {
    ...
    {
        return Excel::download(new PenyakitExport(template: true), 'template-penyakit.xlsx');
    }

    public function exportExcel()
    {
        return Excel::download(new PenyakitExport(), 'data-penyakit-' . now()->format('Ymd_His') . '.xlsx');
    }
    ...
    public function openImportModal(): void
    {
        ...
        $this->modal('penyakit-import-modal')->show();
    }
    ...
    public function importExcel(): void
    {
        ...
        $import = new PenyakitImport();
        Excel::import($import, $this->importFile->getRealPath());

        $failures = $import->failures();

        if ($failures->isEmpty()) {
            Flux::toast(variant: 'success', text: __('Data penyakit berhasil diimpor.'));
        } else {
        ...
        $this->modal('penyakit-import-modal')->close();
    }
};
?>
...
<div class="flex h-full w-full flex-1 flex-col gap-6 p-6">
    ...
    <!-- Modal Import Penyakit -->
    <flux:modal name="penyakit-import-modal" class="md:w-[480px]">
        <form wire:submit.prevent="importExcel" class="space-y-6">
            <div>
                <flux:heading size="lg">{{ __('Import Data Penyakit') }}</flux:heading>
                <flux:text class="mt-2">
                    {{ __('Unggah file Excel sesuai format template. Kolom Menular diisi "Ya" atau "Tidak".') }}
